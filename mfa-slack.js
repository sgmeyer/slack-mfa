...
/**
 * Redirects back to the rule (continuing) for further rule processing.
 **/
function redirectBack(res, webtaskContext, decoded, success) {
  ...
  var token = createSignedToken(webtaskContext, payload);
 ...
 **/
function sendUrlToSlack(req, res, decoded, slackApiToken, slackUsername, slackEnrolled) {
  var payload = {
    sub: decoded.sub,
    aud: decoded.aud
  };
...
function startMfaEnrollment(auth0ApiToken, webtaskDomain, userId, slackUsername) {
  ...
  updateUserData(auth0ApiToken, webtaskDomain, userId, payload);
}

function completeMfaEnrollment(auth0ApiToken, webtaskDomain, userId) {
  var payload = { user_metadata: { slack_mfa_enrolled: true }  };
  updateUserData(auth0ApiToken, webtaskDomain, userId, payload);
}

function updateUserData(auth0ApiToken, webtaskDomain, userId, payload) {
  var options = { method: 'PATCH',
    url: 'https://' + webtaskDomain + '/api/v2/users/' + userId,
    headers: {
...
// Create a signed JWT
function createSignedToken(webtaskContext, payload) {
  var secret = new Buffer(webtaskContext.data.client_secret, 'base64');
  var options = {
      expiresInMinutes: 5,
  ...
  if (!payload.jti) { payload.jti = uuid.v4(); }
  return jwt.sign(payload, secret, options);
}

// Verify a JWT.
function verifyToken(webtaskContext, token, callback) {
  var secret = new Buffer(webtaskContext.data.client_secret, 'base64');
  jwt.verify(token, secret, function(err, decoded) {

    if (err) { return callback(err, decoded); }

    MongoClient.connect(webtaskContext.data.mongo_connection, function(err, db) {
...
function showEnrollmentStep(res, token, slackUsername) {
  res.writeHead(200, {
    'Content-Type': 'text/html'
  });

  res.end(require('ejs').render(hereDoc(enrollmentView), {
      token: token,
...
function showCancelPage(res) {
  res.writeHead(200, {
    'Content-Type': 'text/html'
  });

  res.end(require('ejs').render(hereDoc(cancelView), {}));
}
...
function showErrorPage(res) {
  ...
  res.end(require('ejs').render(hereDoc(errorView), {}));
  return;
}
    ...
    <body>
      <div class="unhandled-error-cont tenant-error-cont ">
        <div class="error-header">
          ...
          <h3 class="error-title">sgmeyer</h3>
        ...
        <div class="error-footer">
          <div class="footer-groups cf">
            <span class="footer-group">
              ...
              <h4 class="footer-group-title">Want to log a bug?</h4>
              <a href="https://github.com/sgmeyer/slack-mfa/issues/new">Post a new issue in our repo.</a>
            </span>
            <span class="footer-group">
              <h4 class="footer-group-title">SUPPORT</h4>
                <p class="footer-group-detail">Please contact the systems administrator.</p>
