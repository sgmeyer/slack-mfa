...
function getMfa(req, res) {
  ...
  jwt.verify(token, secret, function (err, decoded) {
     if (err || decoded.slack_mfa_username) {
       res.status(500).send('Error.' + err).end();
       return;
     }

     var enrollmentToken = createMfaToken(secret, decoded.sub, decoded.aud);
     var baseUrl = process.env.URL || 'https://webtask.it.auth0.com/api/run/' + req.x_wt.container + '/' + req.x_wt.jtn;
     var slackOptions = {
       verifyUrl: baseUrl + '/verify?token=' + enrollmentToken,
       cancelUrl: baseUrl + '/cancel?token=' + enrollmentToken,
     ...
     sendSlackDirectMessage(slackOptions).then(function() {
       res.writeHead(200, {
...
function createMfaToken(secret, sub, aud) {
  var payload = {
    sub: sub,
    aud: aud,
    jti: uuid.v4(),
...
function sendSlackDirectMessage(options) {
